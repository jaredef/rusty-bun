...
Surface drawn from 4 candidate properties across the Bun test corpus. Construction-style: 0; behavioral (high-cardinality): 4. Total witnessing constraint clauses: 61.
...
**Error** — produces values matching the documented patterns under the documented inputs. (behavioral; cardinality 37)

Witnessed by 37 constraint clauses across 5 test files. Antichain representatives:
...
- `test/js/node/v8/capture-stack-trace.test.js:72` — capture stack trace with message → `expect(e2.message === "bad error!").toBe(true)`
