...
**Buffer.alloc** — produces values matching the documented patterns under the documented inputs. (behavioral; cardinality 17)

Witnessed by 17 constraint clauses across 1 test files. Antichain representatives:

- `tests/unit_node/_fs/_fs_raw_fd_test.ts:81` — [node/fs] readSync with position reads at offset without moving cursor → `assertEquals(buf1.toString(), "efg")`
