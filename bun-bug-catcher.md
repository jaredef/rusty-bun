...
- **STILL OPEN**: SHA-384 / SHA-512 (HMAC + raw digest); RSA / ECDSA / AES key-wrap / derive surfaces. SHA-384 and SHA-512 are mechanical pilot extensions (SHA-2 family, 128-byte block, similar to SHA-256 with different word size). RSA/ECDSA require asymmetric-key crypto pilots not yet in the apparatus (substantial work, likely a separate engagement).
