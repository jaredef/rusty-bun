...
- **`N_persist` = 0** (reset this round per the rule: an M8(a) reconciliation occurred — node:os was wired to close part of E.9's compound boundary).
- Prior run reached `N_persist = 6` before deliberate basin-extension: job-queue → batch-loader → log-analyzer → task-pipeline → sequence-id → config-merger (six consecutive zero-reconciliation rounds across orthogonal axes).
- Reset reason: this round chose to *extend the basin* (close node:os boundary, ship system-info fixture using it) rather than *traverse the basin*. The N_persist metric is precise about the distinction — a round that requires apparatus extension resets the count even when the extension lands cleanly. Doc 710 P3 test direction. Next round can begin a new persistence run on in-basin axes if it forgoes basin-extension.
