...
**Status at 2026-05-15: Tier-Ω.5 long-tail convergence + protocol-driven pause point reached.** The hand-roll Tier-Ω trajectory (Ω.3 engine substrate → Ω.4 host-v2 migration → Ω.5 parity re-baseline) executed across ~50 closure + substrate rounds between 2026-05-13 and 2026-05-15. The hand-rolled rusty-js engine substrate is structurally complete for the dominant ES2015–ES2022 surface (parser → AST → bytecode → runtime → host-v2 across 16 named pipelines per Doc 720). Parity against the new engine: **44/118 raw n_ok, ≈47/118 real-substrate-completion** after Doc 721 §VI.5 false-pass correction. The new measurement is honest where the pre-Ω.5 88.2% was *parity-through-rquickjs*; the hand-rolled engine's 44 is *parity-on-its-own-substrate*. The two metrics are not directly comparable — they measure different artifacts.

The engagement reached its **protocol-driven natural pause point** at this state, per Doc 714 §VI Consequence 13. The cross-pipeline diagnostic protocol (Doc 721) returns Step 3 negative on the dominant remaining failure class: the 28-package parse-specific bin's chains converge at fragmented, per-package alphabet tops rather than at a shared dominator. No remaining substrate move has locatable alphabet top in the residual; the long tail is bin-shaped, not substrate-shaped. Continuing past this state requires either (a) per-package debugging methodology, (b) a different evaluation probe that exercises namespace values rather than shape, or (c) a different corpus basket. None is forced by the methodology's terminal value; all are legitimate engagement-extensions if the keeper directs.

**Corpus articulations stacked through the engagement.** Doc 714 §VI Consequences 5 (event-loop architecture inside the engine) → 6 (sub-agent delegation discipline for bounded refactors) → 7 (pipeline empirical landing at Ω.5.f's EventBus fixture) → 8 (substrate-uncovering inventory: bin distribution as engine's sine qua non) → 9 (saturation state: long-tail / substrate-move bifurcation) → 10 (tail-shape pivot to substrate-driven completion) → 11 (top-of-alphabet conjecture: locatability by call-chain walking) → 12 (cross-pipeline diagnostic produces first negative prediction) → 13 (Step 3 negative on the long tail; engagement's natural pause point). Plus Doc 717 (apparatus above engine boundary), Doc 719 (PRESTO ↔ rusty-js-runtime pipeline pattern), Doc 720 (16 interconnected pipelines under SIPE-T topology), Doc 721 (cross-pipeline diagnostic protocol formalized with false-pass correction).

**What the engagement produced.** Two deliverables per Consequence 8, both realized:
1. *A working hand-rolled JavaScript engine* (parser + AST + bytecode + runtime + host-v2 + GC + event-loop + module loader) executing ~47/118 of curated real npm code byte-identical-or-honest-fail against Bun 1.3.11.
2. *The inventory of contingent decisions a JS runtime engine must make*, extracted across 16 pipelines × stable typed-stage signatures, with ~60 decisions made in the codebase and ~30 in deferred-with-clear-error states. The cross-pipeline diagnostic protocol formalized as a portable methodology in Doc 721, with empirical validation record from Ω.5.gg through Ω.5.kk.

The engagement's *protocol-driven phase* is paused at its terminal measurement. Per Doc 714 §VI Consequence 13, this pause is the protocol's terminal value, not a judgment against completeness.
