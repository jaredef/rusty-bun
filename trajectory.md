...
**J.3 — Per-app regression closure.** PARTIAL 2026-05-10: spec-portable surface differentially verified — P_bun = P_drv on URL + URLSearchParams + structuredClone + Buffer.byteLength + TextEncoder/Decoder + atob/btoa + crypto.randomUUID format + Date + JSON + Headers. Bun-asymmetry noted: sync body methods (.text()/.json()) are non-portable; closure of that gap requires async body migration (separate round). Remaining J.3: differentially verify the two existing Tier-J fixtures (consumer-todo-api needs body migration first; consumer-stream-processor mostly portable).
