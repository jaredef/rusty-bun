...
20. **Workspace target-dir redirect requires tool-side honor or symlink bridge.** `.cargo/config.toml` redirects `build.target-dir` to `/media/jaredef/T7/rusty-bun-target` (per the Ω.5.g.2 disk-pressure intervention; the SD card lacks headroom for a ~1.7G workspace debug build). Cargo itself honors the redirect. Any tool that hardcodes `$ROOT/target/release/<bin>` does not. The catching mechanism is the tool exiting with "Build rusty-bun-host first: cargo build --release --bin rusty-bun-host" *after* a clean build has just succeeded — the false negative is the signature.

    **Why visual review fails.** The tool's path string reads as correct; cargo's output reads as correct; the binary exists on disk; the gap is the redirect that lives in a config file the tool does not consult.

    **Catching mechanism.** "Cargo build succeeded but tool says binary is missing" is the signal. The fix is one of three patterns: (a) the tool calls `cargo metadata --format-version 1 | jq -r .target_directory` to learn the actual target path; (b) a symlink at `$ROOT/target → $(target-dir)` bridges the gap for all tools without code changes; (c) the tool reads `.cargo/config.toml`'s `build.target-dir` directly. Pattern (b) is the least invasive and was applied 2026-05-17 to recover parity-measure.sh.

    **Empirical record.** First instance 2026-05-17: parity-measure.sh exit-with-build-prompt after a successful `cargo build --release --bin rusty-bun-host`. Symlinked `target → /media/jaredef/T7/rusty-bun-target` to bridge; parity-measure ran clean immediately after. host/tools/{parity-measure.sh, route2-expanded-probe.sh, route2-value-probe.sh, broaden-basket.sh, probe-builder.sh} and any future tool reading `$ROOT/target/*` are covered by the symlink. If the disk-pressure intervention is ever rolled back, the symlink and the rule both retire together.

    **Composition with §A8.16.** §A8.16 named harness-tier process-global state as a drift source. §A8.20 names the build-artifact path indirection as a related drift source: a process-global configuration that some processes (cargo) honor and others (hardcoded tool paths) do not. Same shape, different stratum.

    **Phase classification.** §A8.20's institution is a Phase-2-extension micro-round per §A8.12. No new alphabet element; the rule extends apparatus operability across the disk-pressure-redirected workspace. Pattern (b) is durable; pattern (a) is the principled fix that should land at any tool that gets revised for other reasons.

21. **Tag-on-DAG: substrate moves are named by coordinate, not by sequence-letter.** The engagement's prior `Ω.5.{letter}` discipline accreted sequentially over time, which produced collisions once the alphabet exhausted (two distinct CCCCCCCC commits with different recognitions: `f3f38deb` callee-shape-probe vs `e8f5aee1` BigInt arithmetic; same shape at DDDDDDDD and EEEEEEEE; visual conflation across 7-letter NNNNNNN and 8-letter NNNNNNNN). The corrective discipline names each move by its position in the engine's structure rather than by its position in time.

    **Form.** `Ω.5.<pipeline>.<layer>.<handle>[.<seq>]` where the coordinates are drawn from the manifest at [`host/tools/dag-coordinates.json`](host/tools/dag-coordinates.json) (manifest_version 1) and the grammar is specified at [`host/tools/tag-grammar.md`](host/tools/tag-grammar.md). Pipelines per [Doc 720 §III](https://jaredfoy.com/resolve/doc/720-the-rusty-bun-runtime-as-a-dag-of-interconnected-pipelines-sipe-t-topology-over-the-engine-substrate) (sixteen, enumerated). Above-engine layers L0–L6 per [Doc 714 §IV sub-§4](https://jaredfoy.com/resolve/doc/714-the-rusty-bun-engagement-read-through-the-lattice-extension-basin-expansion-at-the-l2m-saturation-point). Engine-internal layers E0–E5 per [Doc 717 §V](https://jaredfoy.com/resolve/doc/717-the-apparatus-above-the-engine-boundary-the-three-projections-lifted-to-engine-substrate-and-the-pure-abstraction-point). Per [Doc 716 §II](https://jaredfoy.com/resolve/doc/716-stubs-as-named-cuts-the-three-projection-tracker-and-the-stub-alphabet-stability-conjecture), the tag is the three-projection coordinate read off the substrate node touched.

    **Constraint.** Two distinct moves MUST differ in at least one of (pipeline, layer, handle). Same triple means same recognition, by definition. If two moves at the same triple look distinct, the handle is wrong, not the constraint.

    **Migration discipline.** Existing letter-tags are not renamed. v1 applies from this commit forward. Commit hashes remain canonical across the engagement. The §4 mapping table in the grammar spec carries a hand-curated retro-tag for the EXT 7 stretch as documentation only.

    **Composition with §A8.20.** §A8.20 named one apparatus-operability rule across a stable indirection (the workspace target-dir redirect). §A8.21 names another apparatus-operability rule across a different stable indirection (the substrate's DAG coordinate space against the time-ordered commit log). Same shape, different stratum: both rules institute disciplines that stay operable across an indirection the apparatus has accepted.

    **Phase classification.** §A8.21's institution is a Phase-2-extension micro-round per §A8.12. No new substrate; the rule names a coordinate system over the existing substrate. N_persist resets to 0; the basin is wider by one named naming discipline. Reference: rusty-bun commit instituting this entry; companion artifacts `host/tools/dag-coordinates.json` (manifest v1) and `host/tools/tag-grammar.md` (grammar v1).

22. **Tag-on-DAG operational discipline: validator + append-only log + trajectory column (advisory tier).** The grammar instituted at §A8.21 names the coordinate system; this entry institutes the operability for it. Three artifacts plus one documentation change land together: `host/tools/tag-validate.sh` parses a tag against the manifest and exits nonzero on any malformed segment (unknown pipeline id, unknown layer id, non-kebab handle, numeric-seq smell warning per grammar §2); `host/tools/tag-append.sh` calls the validator, refuses to append on invalid input, refuses to append on a (pipeline, layer, handle) collision against a different commit, and is idempotent on identical (tag, commit) pairs; `host/tools/tag-log.jsonl` is the append-only structured record (one JSON object per line: tag, commit, ISO 8601 timestamp, pipeline, layer, handle, recognition); the trajectory.md EXT 7 table carries a `Tag-on-DAG` column with the worked-example mapping per grammar §4.

    **Advisory tier, by design.** None of the three scripts is wired into a pre-commit hook. Per [Doc 727](https://jaredfoy.com/resolve/doc/727-basin-stability-from-inside) §V's basin-stability concern, tight enforcement of an apparatus discipline reinforces the basin more aggressively than its falsification record currently warrants. The advisory tier runs for one week of observation, then the keeper decides on promotion to enforced. The collision check at the log tier is structural, not enforced: it catches the §A8.21 / grammar §2 constraint by construction at write time, but the writer must be invoked for the catch to fire.

    **Composition with §A8.21 (grammar) and §A8.20 (target-dir redirect).** §A8.20 institutes apparatus operability across the workspace target-dir indirection. §A8.21 institutes a coordinate system over the DAG indirection. §A8.22 institutes the operability for that coordinate system: a parser, an append-log, and a documentation slot. All three rules deal with apparatus operability across stable indirections; §A8.22 specifically supplies the disciplines that make §A8.21 a live record rather than a one-shot spec.

    **Phase classification.** §A8.22's institution is a Phase-2-extension micro-round per §A8.12. No new substrate; no new alphabet element; the rule supplies operability for the coordinate system §A8.21 named. N_persist resets to 0; the basin is wider by one named operational discipline. Reference: rusty-bun commit instituting this entry; companion artifacts `host/tools/tag-validate.sh`, `host/tools/tag-append.sh`, `host/tools/tag-log.jsonl`, and the new trajectory.md `Tag-on-DAG` column at the EXT 7 anchor.

    **Falsifier built for Doc 728 F4.** Each appended log line carries both the prospective tag (the tag named at the move's commit) and, over time, the retrospective re-tag (any later correction to handle granularity, pipeline attribution, or layer assignment). The tag-log makes the convergence between prospective and retrospective tagging measurable across the engagement, which is the F4 observable Doc 728 names: high convergence corroborates substrate-tracking; persistent divergence at specific coordinates localizes basin self-reinforcement at those coordinates.

23. **host-v2 + rusty-js-runtime is the primary substrate target. The rquickjs-backed host/ binary is reference ceiling only.** Per keeper directive 2026-05-17, after Ω.5.P17 (joint await + dynamic-import lift) opened the host-v2 measurement surface, the engagement's substrate-introduction work centers on the hand-rolled engine. New substrate moves land in `pilots/rusty-js-{ast,parser,bytecode,runtime}/derived/src/` and `host-v2/src/`. The legacy `host/` binary continues to compile and continues to participate in the parity sweep as the migration-cost reference per [Doc 717 §VIII](https://jaredfoy.com/resolve/doc/717-the-apparatus-above-the-engine-boundary-the-three-projections-lifted-to-engine-substrate-and-the-pure-abstraction-point), but no new feature work lands there.

    **Rationale.** Through 2026-05-16 evening the engagement carried both binaries forward in parallel because host-v2's parity reading was 0.3% (degenerate; a measurement artifact of the `await` no-op plus the dynamic-import stub keeping the probe blind). Ω.5.P17 closed both cuts as one chain-bundle move per [Doc 721 §III](https://jaredfoy.com/resolve/doc/721-the-cross-pipeline-diagnostic-protocol-locating-the-top-of-a-substrate-widenings-alphabet-by-walking-the-engines-dag); the post-move reading is 40.1% (412/1026) on the parity-top500 sweep, which is the first non-degenerate host-v2 number and the first surface on which the [Doc 726 §VI](https://jaredfoy.com/resolve/doc/726-consumer-embedded-probes-as-an-inherited-layer-d-substrate-the-semiotic-connection-across-residual-clusters) probe-shape × pipeline matrix can classify the host-v2 residual. With the measurement surface open, the engagement's load-bearing substrate work moves to where the cuts now compound. host/ continues at 75.2% (772/1026) on the same basket; the 35-point gap is the migration-cost reading the cut-profile diff predicts.

    **Operational consequence.** Substrate-move proposals name a file:line in the rusty-js crates or host-v2; rquickjs-side proposals require explicit justification (typically: closing an above-engine bug that already independently exists in host/ and surfaces in its parity readings). The parity-measure.sh harness remains the canonical measurement against `host/`; parity-measure-v2.sh (sister script that sets RB_BIN to `target/release/rusty-bun-host-v2`) is the canonical measurement against host-v2. Both sweeps run on the same parity-top500.txt basket so the cut-profile diff is byte-comparable per probe.

    **Phase classification.** §A8.23's institution is itself the M12 move per [Doc 714 §VI.3](https://jaredfoy.com/resolve/doc/714-the-rusty-bun-engagement-read-through-the-lattice-extension-basin-expansion-at-the-l2m-saturation-point): apparatus-tier consolidation at the moment the host-v2 measurement surface opens. N_persist resets is not applicable. The basin is named, not extended. Reference: rusty-bun commits `37609af1` (Ω.5.P17 substrate move), `f7ec71ed` (Ω.5.P17.E0 stdout-fd-routing), `cc283b56` (parity-measure-v2.sh wrapper + post-EXT7 sweep outputs); trajectory.md EXT 8 anchor.

    **Composition with §A8.18.** §A8.18 was the first M12 apparatus-tier consolidation, naming basin-expansion-at-L2M-saturation as the next productive surface when round-tier work exhausted marginal value at the rquickjs ceiling. §A8.23 is the second: it names the substrate-target axis on which subsequent rounds run, after the hand-rolled engine's parity probe became readable. Both rules are about where load-bearing work lives; §A8.18 above the engine boundary, §A8.23 below it.

24. **Telos has two reads. Byte-parity-with-Bun is the headline measurement; swap-without-regression is the load-bearing telos. They diverge on Bun-laxness surfaces, and the divergence is forward progress when host-v2 is the more disciplined reading.** Per keeper recognition 2026-05-17, after the post-P21 walk of the 48-package both-ERR cluster surfaced that every entry was a genuine "package unloadable" case where both engines correctly rejected with different error-wrapper classes. The probe records FAIL because the JSON differs byte-for-byte; the engines agree at the outcome level.

    **The two reads.** The headline number `host-v2 / Bun parity` measures byte-identical-shape against Bun on each package's namespace. The load-bearing telos per seed §V is "a real consumer can swap rusty-bun for Bun without regression," which is satisfied when consumer-observable behavior matches even if the API-edge wrapper classes differ. The first read is a sufficient signal for the second; the second tolerates divergences the first marks as gaps.

    **Where the divergence is forward progress.** Four named instances from the engagement record:
    - **abort-controller** (open, observed 2026-05-17): Bun surfaces non-enumerable `__esModule` in its CJS-as-ESM namespace. The package's source sets `Object.defineProperty(exports, '__esModule', { value: true })`; ECMA §6.2.5.4 CompletePropertyDescriptor defaults enumerable to `false`. Spec-correct enumeration filters it. We do; Bun does not.
    - **Ω.5.P21.E1 (callable-instance-prop filter)** and **Ω.5.P21.E2 (Tuple B drop)**: both engines now produce `{default: fn}` namespaces for ESM-with-only-default; the move converged us to the more disciplined shape, not away.
    - **Ω.5.P19.E1 (Value::Symbol)**: spec-correct typeof "symbol" + spec-correct `Object.keys` filter of Symbol keys per §7.3.21. Where Bun's Symbol typeof is consistent the move is parity-tracking; where Bun is lax the move is improvement-tracking.
    - **48-pkg both-ERR cluster**: both engines correctly reject 48 unloadable packages; the probe FAILs them because Bun wraps in `ResolveMessage` / `Error` and we wrap in `Object`. Engine-observable outcome is identical.

    **Operational discipline added by this entry.**
    - The harness gains a `MATCH_OK_ERR_BOTH` status (both engines produce ERR, regardless of wrapper class) counted as PASS at the headline level. Captures the 48-cluster cleanly without forcing us to mimic Bun's error-class naming.
    - A separate post-hoc classifier surfaces packages where the byte-diff is specifically a `__esModule`-leakage or similar Bun-laxness signature. Marked `IMPROVES_OVER_BUN` for keeper review; not auto-promoted to PASS (the decision is whether to keep spec-correctness or match Bun for ecosystem compatibility).
    - New tag-on-DAG segment available: `.bun-laxness` (or `.improves-over-bun`) for substrate moves whose post-move parity loss is a correctness gain rather than a regression. Surfaces in the trajectory record so future readers see which divergences are disciplines and which are gaps.

    **Composition with §A8.23.** §A8.23 named host-v2 + rusty-js-runtime as the primary substrate target. §A8.24 names that the substrate work has two valid endpoints per surface: match-Bun (when Bun is the reference) and out-spec-Bun (when Bun is the lax case). Both compound with the engagement's substrate-tracking record per Doc 727 §V; both contribute to the load-bearing telos per seed §V.

    **Phase classification.** §A8.24's institution is a Phase-2-extension micro-round per §A8.12. No new substrate; the rule names a measurement-versus-telos distinction that was operating implicitly and now becomes explicit. N_persist resets to 0; the basin is wider by one named methodological discipline. Reference: rusty-bun commits instituting this entry; companion artifacts the `MATCH_OK_ERR_BOTH` harness status and the `classify-improvements.py` post-hoc classifier.

    **Doc 727 cross-check.** Doc 727 §IX flags that the engagement's apparatus-portability is the untested claim. §A8.24 names a sub-claim of that: the apparatus tracks *engine correctness against the spec*, not *engine identity against Bun*. The two coincide where Bun is spec-correct; they diverge where Bun is lax. Bun-laxness instances are predictions of where the apparatus, transported to a non-Bun substrate, would still produce useful diagnostics — they corroborate the portability claim.
...
**Status at 2026-05-17 late evening: Ω.5.P17 joint substrate move opens the host-v2 measurement surface; §A8.23 institutes host-v2 as primary substrate target.** Full trajectory in trajectory.md's RESUME VECTOR EXTENSION 8 anchor. Headline:

- `host/` (rquickjs binary, the legacy ceiling): **75.2% (772/1026)** on parity-top500, post-EXT7. Consistent with the 78-80% @ 846 readings from EXT6 once basket widening absorbs.
- `host-v2` (rusty-js engine binary): **40.1% (412/1026)** post-Ω.5.P17. The pre-P17 reading was 0.3% (degenerate); the +408 raw delta is the chain-bundle close from one joint move. The 35-point gap to host/ is the [Doc 717 §VIII](https://jaredfoy.com/resolve/doc/717-the-apparatus-above-the-engine-boundary-the-three-projections-lifted-to-engine-substrate-and-the-pure-abstraction-point) migration-cost reading: the count of (abstract-op × rung) pairs requiring lift to reach host/'s above-engine maturity from a pure rusty-js base. Post-Ω.5.P18.E1 sweep is in flight; expected to compress that gap by closing the 237-package III.a keyCount-Δ+1 cluster the post-P17 residual surfaced.
- The substrate target axis is now named in §A8.23: host-v2 + rusty-js-runtime is the primary work site; host/ is reference ceiling only.

**Status at 2026-05-16 evening close: thirty-four substrate moves (uuuuuu → BBBBBBBB), five broadenings to 846 packages, two corpus-tier articulations (Docs 725 + 726).** Trajectory detailed in trajectory.md's RESUME VECTOR EXTENSION 6 anchor. Headline movements:
