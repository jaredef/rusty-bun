...
Surface drawn from 2 candidate properties across the Bun test corpus. Construction-style: 1; behavioral (high-cardinality): 1. Total witnessing constraint clauses: 66.
...
Witnessed by 53 constraint clauses across 3 test files. Antichain representatives:

- `test/js/node/fs/fs.test.ts:1555` — writeFileSync > returning Buffer works → `expect(buffer[i]).toBe(out[i])`
- `test/js/node/buffer.test.js:1381` — buffer → `expect(buf.write("hello world ")).toBe(12)`
