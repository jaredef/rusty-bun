...
@provides: bundler-surface-property
  threshold: BUND1
  interface: [S, Reflect.getMetadata, out.includes, metaFile.text, result3.outputs, code.includes, napiModule.getCompilationCtxFreedCount, Object.keys]

@imports: []

@pins: []

Surface drawn from 13 candidate properties across the Bun test corpus. Construction-style: 0; behavioral (high-cardinality): 13. Total witnessing constraint clauses: 114.
...
**out.includes** — produces values matching the documented patterns under the documented inputs. (behavioral; cardinality 9)
...
**imports.filter** — satisfies the documented containment / structural-shape invariants. (behavioral; cardinality 6)

Witnessed by 6 constraint clauses across 1 test files. Antichain representatives:

- `test/bundler/transpiler/transpiler.test.js:1602` — Bun.Transpiler > scanImports > reports import paths, excluding types → `expect(imports.filter(({ path }) => path === "remix")).toHaveLength(1)`
- `test/bundler/transpiler/transpiler.test.js:1603` — Bun.Transpiler > scanImports > reports import paths, excluding types → `expect(imports.filter(({ path }) => path === "mod")).toHaveLength(0)`
- `test/bundler/transpiler/transpiler.test.js:1604` — Bun.Transpiler > scanImports > reports import paths, excluding types → `expect(imports.filter(({ path }) => path === "react")).toHaveLength(1)`
...
**metafile.inputs** — is defined and resolves to a non-nullish value at the documented call site. (behavioral; cardinality 6)
...
- `test/bundler/metafile.test.ts:610` — Bun.build metafile option variants > metafile: { markdown: path } writes markdown to speci… → `expect(metafile.inputs).toBeDefined()`
...
**metafile.outputs** — is defined and resolves to a non-nullish value at the documented call site. (behavioral; cardinality 5)
...
- `test/bundler/metafile.test.ts:558` — Bun.build metafile option variants > metafile: string writes JSON to file path → `expect(metafile.outputs).toBeDefined()`
- `test/bundler/metafile.test.ts:648` — Bun.build metafile option variants > metafile: { json: path, markdown: path } writes both … → `expect(metafile.outputs).toBeDefined()`
...
**output.includes** — produces values matching the documented patterns under the documented inputs. (behavioral; cardinality 5)

Witnessed by 5 constraint clauses across 1 test files. Antichain representatives:

- `test/bundler/transpiler/transpiler.test.js:1138` — Bun.Transpiler > exports.replace > deletes dead exports and any imports only referenced in… → `expect(output.includes("loader")).toBe(false)`
- `test/bundler/transpiler/transpiler.test.js:1139` — Bun.Transpiler > exports.replace > deletes dead exports and any imports only referenced in… → `expect(output.includes("react")).toBe(false)`
- `test/bundler/transpiler/transpiler.test.js:1140` — Bun.Transpiler > exports.replace > deletes dead exports and any imports only referenced in… → `expect(output.includes("action")).toBe(true)`
