...
Surface drawn from 3 candidate properties across the Bun test corpus. Construction-style: 0; behavioral (high-cardinality): 3. Total witnessing constraint clauses: 242.
...
**SQL** — produces values matching the documented patterns under the documented inputs. (behavioral; cardinality 218)

Witnessed by 218 constraint clauses across 3 test files. Antichain representatives:
...
- `test/js/sql/adapter-env-var-precedence.test.ts:82` — SQL adapter environment variable precedence > should not prioritize DATABASE_URL over expl… → `expect(options.options.adapter).toBe("postgres")`
...
**SQL.PostgresError** — produces values matching the documented patterns under the documented inputs. (behavioral; cardinality 15)
...
**SQL.SQLiteError** — produces values matching the documented patterns under the documented inputs. (behavioral; cardinality 9)
