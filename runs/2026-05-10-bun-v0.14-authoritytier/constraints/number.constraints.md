...
Surface drawn from 6 candidate properties across the Bun test corpus. Construction-style: 0; behavioral (high-cardinality): 6. Total witnessing constraint clauses: 62.
...
**Number** — satisfies the documented invariant. (behavioral; cardinality 17)
...
- `test/regression/issue/24045.test.ts:12` — assert.deepStrictEqual() should compare Number wrapper object values - issue #24045 → `assert.deepStrictEqual(new Number(1), new Number(1))`
