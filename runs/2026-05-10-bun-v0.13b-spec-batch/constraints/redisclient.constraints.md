...
**RedisClient** — produces values matching the documented patterns under the documented inputs. (behavioral; cardinality 12)

Witnessed by 12 constraint clauses across 3 test files. Antichain representatives:
...
- `test/js/valkey/reliability/recovery.test.ts:38` — client.connect() recovers after the client enters the failed state → `expect(client.connected).toBe(true)`
