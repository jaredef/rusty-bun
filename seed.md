...
The engagement through 2026-05-13 (commit `013041ea`) achieved an extended form of **curated-corpus parity**: 593 J.1.a fixtures (577 inner-loop + 16 slow-burst) byte-identical to Bun 1.3.11 across the production-shape vendored library catalog, real HTTP/HTTPS server, real WebCrypto with full algorithmic surface (including EC keypair generation), and a heavy basket of real npm packages (express, koa, jose, zod, dayjs, lodash, debug, nanoid, ulid, hono partial, fastify partial, plus tier-A canonical packages from large engagement sweeps). The L2/L3 spec-derived enumerator runs at 100% parity with Bun (418/418). The curated corpus demonstrates the apparatus works under heavy load; it does not by itself demonstrate that any arbitrary Bun app drops in.

The 2026-05-11 → 2026-05-13 slice closed the network tier (HTTP, DNS, gzip/deflate, brotli, full TLS 1.3, WebSocket), the runtime-model tier (auto-keep-alive, process events, cooperative-loop yield), and the load-bearing node:* breadth (events/stream/util/querystring/url/tls/net/assert). Per Doc 715 §VII operational shift 3 + Doc 714 sub-§4.e, the apparatus has moved past the substrate-introduction phase into the consolidation regime: empirical retirement-fanout 50-200× per substrate widening, new-substrate-edges-exercised per probe ≈ 0.3, zero new alphabet elements across the last ~50 consumer probes (P2 leaf-set-bounded corroborated).

The gap from this extended curated parity to full plug-and-play parity is now dominated by three terms, not the original 12-19-round substrate list:
1. **Real-OSS basket expansion** — canonical third-party packages beyond the J.1.a basket; estimated mechanical at current density coefficient.
2. **Cooperative-loop reactor (E.18/E.19 class)** — the 8-microtask-burst yield holds for canonical frameworks (express + 6-middleware + polka + 12-deep handler chain); substantially-deeper async chains or truly-concurrent connections would need a real mio/epoll reactor. 3-5 rounds if attempted, high-cascade value.
3. **eval-ESM (prettier class)** — module-context dynamic eval on bundled minified source with top-level `export`. 2-3 rounds, low-cascade value.

The original substrate-introduction list (TLS, real-network fetch, compression, DNS, WebSocket, async-runtime-model auto-listen, node:* breadth) is fully retired.
...
- **Current state (2026-05-13):** http-codec (RFC 7230 HTTP/1.1) + sockets (TCP with async setNonblocking + tryRead + __tickKeepAlive cooperative-yield) + Bun.serve facade with listen()/tick()/serve()/autoServe + TLS 1.3 substrate (~2625 LOC pure Rust: ASN.1/DER → X.509 → record layer → AEAD → ClientHello/ServerHello → key schedule → driver → chain walk → live-verified against openssl) + WebSocket (RFC 6455 frame codec + handshake key derivation + JS-side WHATWG WebSocket class + live ws:// round-trip against Bun-spawned echo server) + compression (gzip/deflate decode hand-rolled per RFC 1951/1952/1950; gzip/deflate stored-block encode; brotli decode via borrowed substrate). Same-process server+fetch round-trip works via cooperative-yield reactor. **Transport tier is functionally complete for sub-2/sub-5 work.**
- **Full-parity remaining:**
  - **Real mio/epoll reactor** — cooperative-yield 8-microtask-burst suffices for canonical real-OSS depth; substantially-deeper async or true-concurrent-connections require a real reactor. 3-5 rounds if attempted.
  - **HTTP/2 multiplexing** — out-of-scope for v1; HTTP/1.1 fallback covers nearly all servers.
  - **IPC / Bun.spawn full stdio streaming** — partial; signal-driven coordination deferred until a consumer surfaces.

**Sub-criterion 4 — JS host integration.** Embed a JS engine (QuickJS or Boa) and expose all pilots to JS code via FFI.
- **Current state (2026-05-13):** rquickjs embedded with 8MB runtime stack ceiling; ~25 pilot families wired; CommonJS + ESM module loaders honor relative + bare-specifier resolution with node_modules walk-up + `node:*` builtin scheme + wildcard subpath exports + main-as-dir resolution + .mjs/.cjs file-extension-decides-module-type gate; createRequire(url) URL-dirname-bound; import.meta.url propagates to FsLoader-loaded modules; timers + queueMicrotask + performance + URL globals wired; Buffer as Proxy-wrapped Uint8Array subclass (apply+construct traps, statics-enumerable, constructable); auto-keep-alive via globalThis.__keepAlive Set + __tickKeepAlive drained by eval loop (Π2.6 closed); process EventEmitter pattern with signal-stub handlers (Π2.7 closed); cooperative-loop 8-microtask-burst yield in fetch WouldBlock path (E.18/E.19 retired).
- **Full-parity remaining:**
  - **Cooperative-loop reactor ceiling** — 8-yield holds for canonical real-OSS depth; deeper async chains or true-concurrent connections need mio/epoll. Scoped to sub-3 transport.
  - **WebAssembly** — out of scope for v1 (QuickJS doesn't include a WASM engine).
  - **eval-ESM (module-context dynamic eval)** — prettier-class consumers use indirect-eval on bundled minified ESM with top-level export; 2-3 rounds, low cascade.
  - **Real signal delivery to JS** — registration stubs in place; SIGINT/SIGTERM actually firing JS handlers deferred until a consumer surfaces it.
...
- **Current state (2026-05-13):** 593 J.1.a fixtures byte-identical to Bun 1.3.11 (577 inner-loop + 16 slow-burst). Coverage spans the production-library shape catalog (~20 orthogonal vendored-library axes) + WebCrypto closure (digest/HMAC/PBKDF2/HKDF/AES-GCM/CBC/CTR/KW/RSA-OAEP/PSS/PKCS1v15/ECDSA/ECDH P-256/P-384/P-521 + keygen) + Tier-G HTTP/TCP/async-bridge + full TLS 1.3 over real openssl + WebSocket live round-trip + real-OSS basket (express, koa, jose, zod, dayjs, lodash, debug, nanoid, ulid, plus tier-A sweeps). All differential-verified.
- **Full-parity remaining:** mostly mechanical basket expansion at the current density coefficient (50-200× retirement-fanout per substrate widening, 0.3 new-substrate-edges-per-probe in the instance-fill regime). The three structural blockers are: (a) the cooperative-loop reactor question (E.18/E.19 class — handled by 8-microtask-burst yield for canonical depth, real mio/epoll reactor required for deeper async or true concurrency); (b) eval-ESM (prettier class — module-context dynamic eval on bundled minified ESM source); (c) basket-coverage of canonical third-party packages not yet probed (hono v4 full surface, elysia, drizzle-orm, prisma-client — each is single-fixture-per-round at current density).
...
**Status at 2026-05-13.** The original Phase Π1-Π4 substrate-introduction roadmap (estimated 12-19 rounds at session-start) is **fully retired**: network tier (Π1.1-Π1.5) + runtime-model (Π2.6-Π2.7) + node:* load-bearing breadth (Π3.8-Π3.13) + Bun-namespace load-bearing subset (Π4.14, Π4.14.5, Π4.14.6, Π4.14.7, Π4.16-extension, Π4.16-small) all closed. Per Doc 715 §VII shift 3, the apparatus has crossed L2M-saturation into the consolidation regime: empirical retirement-fanout 50-200× per substrate widening, new-substrate-edges-per-probe ≈ 0.3. The roadmap below is preserved as historical record. The live forward roadmap is held in trajectory.md "Remaining work toward telos closure (2026-05-13)" — three terms (real-OSS basket / cooperative-loop reactor / eval-ESM), dominated by basket expansion rather than substrate introduction.
