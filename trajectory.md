...
- **`N_persist` = 6** (rounds without M8(a) reconciliation across orthogonal axes).
- Run: job-queue → batch-loader → log-analyzer → task-pipeline → sequence-id → (this, config-merger: Array.toSorted/toReversed/toSpliced/with + Promise.withResolvers + Object.groupBy + structuredClone-TypedArray + deep-spread).
- **Basin boundaries recorded:**
  - E.7 — WeakRef/FinalizationRegistry absent
  - E.8 — crypto.subtle.importKey/sign/verify absent
  - E.9 — Intl + WebSocket + BroadcastChannel + Worker + Bun.password/sql absent (compound)
  - E.10 — Set.union/intersection/difference (ES2025) absent (this round's probe; easy polyfill candidate)
- **Multi-op rounds achieved without coherence loss:** K=2 at `9c67ac6`; K=2 at `88bbd54`; K=3 at `ec17f84`; K=2 at (this).
- Axes-set covered by the persistence run: ~47 distinct surface idioms across six rounds.
- In-basin discoveries (cumulative across probes): Atomics + SharedArrayBuffer + WeakMap + WeakSet + Symbol.asyncIterator + Promise.withResolvers + Array immutable methods (ES2023) + Object.groupBy + structuredClone-TypedArray + Atomics.wait/notify.
- Next round predicts: K=2-3 sustainable; basket-expansion continues monotonically; basin's known boundary continues to tighten. Reset to 0 on M8(a). Doc 710 P5 (basin saturation) NOT yet hit — probe surface remains.
