...
**J.1 — Application basket.** Curate a representative basket of Bun-using applications: Hono / Elysia frameworks at example-app level; npm packages whose test suites Bun runs cleanly; Cloudflare Workers examples; small but real-world apps. PARTIAL 2026-05-10:
  - `consumer-todo-api/` (ESM, Bun.serve-based, todo CRUD; 10/10 self-tests; compositional vacuity).
  - `consumer-stream-processor/` (CJS, async-heavy stream pipeline + AbortController + setTimeout + fs; 8/8 self-tests; scope-limit verified — cross-stream abort cascade not implemented, deferred per streams round).
