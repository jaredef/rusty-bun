...
Sub-categorized by differential portability per M8 (instituted 2026-05-10):
  ...
  **J.1.b — rusty-bun-internal (not Tier-J differential):** fixtures that exercise the host's wired pilots but depend on rusty-bun-only shapes. Per M8, removed from differential set; kept as host-internal regression tests. Re-open condition: when the divergent shapes are reconciled with Bun (see per-fixture re-open).
   - `consumer-stream-processor/` (CJS, ReadableStream/TransformStream/WritableStream pipeline + AbortController + setTimeout + fs + Headers + Buffer + URL; 8/8 self-tests on rusty-bun-host).
     - **Divergences**: depends on global `fs` namespace (rusty-bun-host wires it; Bun requires `import fs from "node:fs"`), `Buffer.encodeHex` (rusty-bun-host static helper; Bun has Node Buffer instance `.toString("hex")`), and the `globalThis.__asyncResult` host-internal result-passing path (Bun has no equivalent).
     - **Re-open condition** (3 sub-items, post `b03be42`+next):
       - ~~(i) module loader supports `node:fs` bare specifier resolution~~ ✅ closed (NODE_BUILTINS table in CJS loader; node:fs/path/http/crypto/buffer all resolve).
       - ~~(ii) Buffer wrapped as a class with `.toString(encoding)` instance method~~ ✅ closed (BUFFER_CLASS_JS extends Uint8Array).
       - (iii) fixture rewritten to write its result via process.stdout.write + use `require("node:fs")` + use `Buffer.from(...).toString("hex")` — REMAINING; once done, fixture moves from J.1.b to J.1.a.
...
**J.3 — Per-app regression closure.** PARTIAL 2026-05-10:
  - Spec-portable surface differentially verified — P_bun = P_drv on URL + URLSearchParams + structuredClone + Buffer.byteLength + TextEncoder/Decoder + atob/btoa + crypto.randomUUID format + Date + JSON + Headers (`6187bc7`).
  - consumer-todo-api differentially verified post-M8 — byte-identical "10/10" under Bun 1.3.11 and rusty-bun-host (this commit).
  - Remaining: differentially verify consumer-stream-processor (likely needs Bun-shape adjustment for streams; per M8 will be done in-round when attempted).
