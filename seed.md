...
**Status at 2026-05-16 early morning: closing-problems regime + Doc 724 publication + JIT planning entry.** The expanded-route-2 value-exercise probe (71 packages) sits at **64/71 (90.1%) byte-identical to Bun under value-exercise**, err=5, two BADs. Up from the prior anchor's 47/71 (66%). Closed twenty-six substrate moves (Ω.5.jjjj through Ω.5.cccccc) plus four deep-trace tag widenings (Ω.5.xxxxx/yyyyy/yyyyy.b/hhhhh). Three bisects landed via the route-(b) → bisect → fix loop (ts-pattern → block-scope; moment → substr; marked → named-class self-binding). Each bisect ended at a single-line ECMA spec compliance gap.

The remaining frontier 5 errors require deep engine features (generators for superstruct; real Proxy interception for immer; readable-stream subclass chain for ndjson) or library-internal multi-hop bisects (pako deflate state machine; micromark state machine).

Corpus articulations stacked another doc this stretch: **Doc 724 — Feature-Set Prediction: Static Substrate-Need Mapping from Source**, formalizing the keeper's conjecture that the bidirectional traceability property (Doc 714 C14) supports forward prediction of substrate requirements from source AST alone, before execution. Doc 724 names that the fifty backward-walks completed in the engagement were reading the same map a single forward-walk would read once; the forward direction is cheaper and is the testable form of the discipline.

**Keeper directive (2026-05-16): JIT entry.** The runtime currently runs bytecode in a tree-walking match-on-Op interpreter. Hot-path costs dominate at Op::GetProp (proto walks on every read; no inline caches), Op::CallMethod (re-resolves the function every call), Op::GetIndex (string-key conversion via property_key per element), and numeric arithmetic (boxed Value through to_number on every op). Three candidate JIT shapes per trajectory §EXT3: (1) inline-cache layer first; (2) baseline JIT via Cranelift; (3) hand-rolled template JIT (Pin-Art-rigorous). The Doc 724 forward-predictor naturally extends to JIT planning — each Op site the predictor names as hot is an IC / specialization candidate, coverage-frequency-sortable.
