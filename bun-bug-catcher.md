...
- **STILL OPEN**: ECDSA / ECDH over P-384 / P-521. These need additional curve parameters and re-parameterization of the EC substrate (the curve operations are isomorphic across these short-Weierstrass curves with a=-3; a single `Curve` struct carrying p, n, a, b, G + corresponding doubled/add closures would close all four remaining surfaces in one round). RSA/ECDSA/ECDH need big-integer + finite-field-curve primitives. AES-KW/CBC/CTR are easy follow-ons to the existing AES block cipher. HKDF is straightforward given the HMAC family.
