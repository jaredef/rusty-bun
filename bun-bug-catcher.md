...
### F12. Parallel cargo tests racing on a process-global env var route fixture traffic to the wrong server
- **The rule.** When an integration test sets a process-global env var (e.g., `FETCH_TEST_PORT`) to point a fixture at a per-test in-process server, parallel test execution (cargo's default) lets two tests overwrite the env var concurrently. The second test's fixture connects to the first test's port (or vice versa), the first test's server accepts client connections it expected from its own fixture but never reaches its bound count, and the harness deadlocks at `server_thread.join()`. Visible symptom is identical to F9 (silent 0% CPU hang at end of test) but the root cause is the env-var race, not the accept-count.
- **Why visual review fails.** Each test reads correctly in isolation: bind ephemeral port, set env var, run fixture, join. The race is invisible without thinking about parallel test scheduling. Single-threaded runs (`--test-threads=1`) pass; default parallel runs hang. F9-recovery instincts (re-check the accept bound, re-check the fixture's connection count) come up empty because the bound is correct.
- **Catching mechanism.** If a test passes under `--test-threads=1` but hangs under default parallel `cargo test`, the suspect is a shared mutable resource: env vars, well-known port numbers, on-disk fixtures with a fixed path, global statics. Env vars are the most common in this engagement's harness style.
- **Incident.** real_fetch tests, 2026-05-11 (Π1.1 closure follow-up). Single-test runs passed; full-suite parallel runs hung silently for 30+ minutes at 0% CPU. Diagnosis via `--test-threads=1` showed clean 2/2 pass in 0.12s, locating the race to parallel scheduling. Fixed by wrapping `with_fetch_target_server` in a `static Mutex<()>` whose guard is held across `set_var → f() → remove_var → join`, serializing the env-var-and-port use across parallel test threads. The mutex is `static` (not per-call) so distinct fetch-target-server users contend on the same lock.
- **Generalization.** Any harness pattern that mutates process-global state (env vars, working directory, signal handlers, FD-numbered well-known endpoints) needs a serial guard, OR the global state must be replaced with per-test injection (config struct threaded through the call). The mutex is a one-line fix; the injection refactor is cleaner but bigger. Use the mutex first; consider the refactor if more global-state-using tests accumulate.

### F11. rquickjs async-function returning sync-FFI-result hangs/desyncs the runtime
- **The rule.** When an FFI call is purely synchronous and returns a value immediately, wrapping the call in an `async` JS function (so the caller `await`s it) causes a hang or desync under rquickjs's Promise/microtask flow when the wrapping async function is called inside another async function. Prefer keeping FFI wrappers synchronous unless the underlying operation is genuinely async (yields control).
- **Why visual review fails.** Real Bun's API surface (e.g., Bun.serve) returns servers that are "already listening" — the consumer never awaits a listen step. Adding `async` to mimic Bun's "always-await-able" feel seems harmless. The bug: rquickjs's microtask drain between the async function's return and the await's continuation appears to interact pathologically with the underlying FFI state (specific mechanism not yet diagnosed). Either the listener is dropped, the kernel listener fd is closed, OR the runtime gets stuck somewhere that prevents subsequent FFI calls from resolving correctly.
- **Catching mechanism.** Run the fixture; if a TCP connect to a freshly-listened port returns ECONNREFUSED, OR the test hangs at 0% CPU after the bind step, the async-wrapper-around-sync-FFI is the suspect. Convert the wrapper to sync and rerun.
- **Incident.** Bun.serve facade `server.listen()` round 2026-05-11. Initial implementation was `async listen()`. Calling `await server.listen()` from selfTest's async body led to: case 3's connect refused (kernel had nothing listening on the just-bound port), eventually a full test hang. Made listen() sync → connect succeeded, tests pass. The mechanism remains under-diagnosed but the rule is empirically clear: don't wrap sync-resolving FFI in async-shape unless you have a real reason to.
- **Generalization.** Any time you find yourself adding `async` to a JS method that doesn't actually await anything, stop and use the sync signature. The minor caller-side inconvenience (no `await` keyword) is worth the runtime stability.

### F10. Registry-lock + background-thread-mutation deadlock
- **The rule.** When an FFI handle owns a blocking primitive (mpsc::Receiver, condvar, etc.) AND a background thread wants to mutate adjacent state through the same registry mutex, the blocking primitive MUST live behind its own `Arc<Mutex<>>` so the registry lock can be released BEFORE blocking. Otherwise: foreground holds registry-lock + blocks on recv; background needs registry-lock to send; deadlock.
- **Why visual review fails.** The locking pattern looks natural ("hold lock, dispatch on handle kind, do the work"). The deadlock surfaces only when the background thread actually fires AND the foreground actually waits. Per-method tests with the foreground returning quickly mask it.
- **Mitigation.** Clone the Arc out of the registry, release the registry lock, then block on the cloned primitive. This is the std-only equivalent of Bun's Waker mechanism (interrupts a polled wait without holding global state).
- **Incident.** rusty-sockets pilot async-listener_poll 2026-05-11. Initial implementation held `registry().lock()` during `rx.recv_timeout()`. The accept-loop background thread tried to `put(Handle::Stream)` (needs registry-lock) to publish a Connection event but blocked, so recv_timeout always saw an empty channel and returned None. Caught by the async_listener_delivers_connection_event pilot test on first run; fixed by wrapping rx in `Arc<Mutex<Receiver>>`.

### F9. Integration-test server-thread accept-loop deadlock-at-join
- **The rule.** When an integration test spawns a server thread that calls `listener.accept()` in a loop, the accept count MUST match the number of connections the fixture actually opens. An unbounded or over-bounded accept loop will block forever when the fixture finishes early, and the test deadlocks at `server_thread.join()`.
- **Mitigations.** Either (a) bound the accept loop to the known connection count, (b) use `listener.set_nonblocking(true)` + poll with timeout + a shutdown signal, or (c) drop the listener from the calling thread to interrupt accept (causes EBADF on the server side). (a) is simplest when the fixture's connection pattern is known and stable.
- **Why visual review fails.** "Just loop accept(); fixture connects N times; should work" reads sensibly. The deadlock surfaces only when the test is actually run; cargo's per-test timeout (default: unlimited) doesn't catch it. The previous round's `with_echo_server` happened to set `for _ in 0..3` matching the fixture's three reads — purely by luck. The http-over-tcp fixture had a 4-connection pattern; first attempt used `for _ in 0..6` to be "safe" and deadlocked.
- **Incident.** consumer-http-over-tcp-suite integration test 2026-05-11. `for _ in 0..6` accept loop; fixture connected 4 times; 5th accept blocked forever; `server_thread.join()` waited forever. Killed the running test, changed to `for _ in 0..4`. Recorded so future server-thread harnesses use the exact connection count.

### F8. rquickjs Vec<u8> binding rejects Uint8Array via Array.from
- **The rule.** When converting a typed-array (Uint8Array, Buffer) input to a `Vec<u8>`-bound FFI parameter in rquickjs, use explicit index-iteration `for (let i = 0; i < bytes.length; i++) arr.push(bytes[i])` — NOT `Array.from(bytes)`. The latter silently produces an Array that rquickjs converts to a Vec<u8> with unexpected contents (cause not fully diagnosed; possibly a typed-array spread/iteration mismatch in QuickJS's prototype chain).
- **Why visual review fails.** `Array.from(uint8Array)` is canonical spec'd JS and works in every other runtime. The bug is silent: the FFI call succeeds (no JS error at call site) but the Rust side sees bytes that don't match the input.
- **Catching mechanism.** Run a smoke test calling the FFI with both Array-from and direct-iteration; compare what Rust sees on each path. Same general class as F4/F5/F6/F7: silent-output-wrong, caught only by comparator differential.
- **Incident.** consumer-http-codec-suite chunkedDecode call 2026-05-11. Direct call `__httpCodec.chunkedDecode(Array.from(bytes))` succeeded; wrapper call `__httpCodec.chunkedDecode(Array.from(bytes))` (same expression, same scope) failed with "missing chunk-size CRLF" — Rust received bytes lacking CRLF. Fixed by switching the wrapper to direct for-loop iteration. Rust side received correct bytes after the change.
- **Generic application.** Every wrapper crossing typed-array → `Vec<u8>` FFI must use direct iteration. Codified in wire_http_codec's `toByteArray(b)` helper at host/src/lib.rs.
