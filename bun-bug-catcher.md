...
- **STILL OPEN**: RSA-OAEP / RSA-PSS / ECDSA / ECDH surfaces (asymmetric crypto requiring big-integer + finite-field-curve primitives). RSA/ECDSA/ECDH need big-integer + finite-field-curve primitives. AES-KW/CBC/CTR are easy follow-ons to the existing AES block cipher. HKDF is straightforward given the HMAC family.
