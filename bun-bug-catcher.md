...
- **STILL OPEN**: ECDSA P-384 / P-521 + ECDH (P-256/P-384/P-521). ECDH is mechanically a single scalar-multiplication operation on the existing P-256 substrate (cheap follow-on for P-256); P-384/P-521 need additional curve parameters and re-parameterization of the scalar-mul code (the curve operations are isomorphic across these short-Weierstrass curves with a=-3). RSA/ECDSA/ECDH need big-integer + finite-field-curve primitives. AES-KW/CBC/CTR are easy follow-ons to the existing AES block cipher. HKDF is straightforward given the HMAC family.
