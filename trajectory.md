...
| 2026-05-10 | (this) | **Tier-H.4 #1: timers + queueMicrotask + performance landed** (setTimeout/setImmediate/clearTimeout, queueMicrotask, performance.now/.timeOrigin; pilot scope: timers as microtasks, real wall-clock delay deferred; 167/167 host tests; 758/758 workspace tests; M7 fold-back: **vacuous** — fourth consecutive non-primitive round; SIPE-T steady-state run extending) |
...
**H.4 — globalThis setup.** PARTIAL: setTimeout/setImmediate/clearTimeout, queueMicrotask, performance.now/.timeOrigin landed 2026-05-10 (microtask-scoped timers; real wall-clock delays deferred). Remaining: URL (WHATWG, ~spec-formalization Pattern 4 against the same constraint set as URLSearchParams), URLSearchParams already wired. fetch (already wired in H.2). console and structuredClone wired in H.2. Specific gaps: URL class, real-time setInterval, AbortSignal.timeout, performance.mark/measure.
