...
- **STILL OPEN**: RSA-PSS / ECDSA / ECDH surfaces. RSA-PSS is a near-follow-on now that bigint + MGF1 are in place (PSS encoding/verification + the same modexp). ECDSA/ECDH need finite-field-curve primitives (P-256/P-384/P-521 — substantial separate work, ~1500 LOC of curve arithmetic). RSA/ECDSA/ECDH need big-integer + finite-field-curve primitives. AES-KW/CBC/CTR are easy follow-ons to the existing AES block cipher. HKDF is straightforward given the HMAC family.
