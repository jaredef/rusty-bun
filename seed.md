...
The engagement through 2026-05-13 evening (commit `e2f15797`) achieved an extended form of **curated-corpus parity**: 598 J.1.a fixtures (582 inner-loop + 16 slow-burst) byte-identical to Bun 1.3.11 across the production-shape vendored library catalog, real HTTP/HTTPS server, real WebCrypto with full algorithmic surface, a heavy basket of real npm packages (express, koa, jose, zod, dayjs, lodash, debug, nanoid, ulid, hono partial, fastify partial), plus the cooperative-loop reactor closure round (mio-driven fetch + listener + TLS + WebSocket + external-fanout sources). The L2/L3 spec-derived enumerator runs at 100% parity with Bun (418/418).

The 2026-05-11 → 2026-05-13 slice closed the network tier (HTTP, DNS, gzip/deflate, brotli, full TLS 1.3, WebSocket), the runtime-model tier (auto-keep-alive, process events, cooperative-loop reactor), the load-bearing node:* breadth (events/stream/util/querystring/url/tls/net/assert), and — in the 2026-05-13 evening Π2.6.c + Π2.6.d sweep — the cooperative-loop reactor ceiling work: nine sub-rounds (c.a reactor MVP / c.b fetch tryRead / c.c listener accept / c.d TLS pump / c.e WebSocket pump / d.a inotify file watchers / d.b child-process pipes / d.c signalfd signals / d.d eventfd async DNS) collectively wiring a single mio reactor with eight heterogeneous I/O source classes registered against a partitioned Token namespace. Per Doc 715 §X consolidation amendment, the reactor's in-degree from substrate-class consumers (eight direct dependents in one session) is the highest of any node the engagement has constructed — corroborating P1 at the substrate-internal scale.

Per Doc 715 §VII operational shift 3 + Doc 714 sub-§4.e, the apparatus has moved past the substrate-introduction phase into the consolidation regime: empirical retirement-fanout 50-200× per substrate widening, new-substrate-edges-exercised per probe ≈ 0.3, zero new alphabet elements across the entire Π2.6.c + Π2.6.d sweep (each external-fanout source instantiates the existing "fd-readable" alphabet element; no new edge kinds).

The gap from this extended curated parity to full plug-and-play parity is now dominated by **two terms**, not the original 12-19-round substrate list:
...
2. **eval-ESM (prettier class)** — module-context dynamic eval on bundled minified source with top-level `export`. 2-3 rounds, low-cascade value.

The original substrate-introduction list (TLS, real-network fetch, compression, DNS, WebSocket, async-runtime-model auto-listen, node:* breadth, cooperative-loop reactor) is fully retired. The five-surface cooperative-loop ceiling analysis from earlier in this engagement maps onto DAG depth from the substrate (Doc 715 §X.c): four surfaces at depth ≤ 2 from the reactor (handler-depth, true-concurrency, external-fanout, CPU-during-idle) close cheaply via mio integration; the fifth (adversarial-async-graph) sits at depth ~∞ from the reactor — it requires hand-rolling the JS engine's scheduler and is a successor-engagement target rather than an incremental round.
...
- **Current state (2026-05-13 evening):** 598 J.1.a fixtures byte-identical to Bun 1.3.11 (582 inner-loop + 16 slow-burst). Coverage spans the production-library shape catalog (~20 orthogonal vendored-library axes) + full WebCrypto + Tier-G HTTP/TCP/async-bridge + full TLS 1.3 over real openssl + WebSocket live round-trip + real-OSS basket (express, koa, jose, zod, dayjs, lodash, debug, nanoid, ulid, plus tier-A sweeps) + cooperative-loop reactor empirical anchors (32-await handler depth, mio-driven inotify watcher, async spawn pipe round-trip, signal handler delivery, async DNS resolution). All differential-verified.
- **Full-parity remaining:** mostly mechanical basket expansion at the current density coefficient (50-200× retirement-fanout per substrate widening, 0.3 new-substrate-edges-per-probe). The two structural blockers are: (a) eval-ESM (prettier class — module-context dynamic eval on bundled minified ESM source); (b) basket-coverage of canonical third-party packages not yet probed (hono v4 full surface, elysia, drizzle-orm, prisma-client — each is single-fixture-per-round at current density). The cooperative-loop reactor question that previously sat in this list has been retired structurally by the Π2.6.c + Π2.6.d sweep.
...
**Status at 2026-05-13 evening.** The original Phase Π1-Π4 substrate-introduction roadmap (estimated 12-19 rounds at session-start) is **fully retired**, and the cooperative-loop reactor work (Π2.6.c.a-e + Π2.6.d.a-d, nine substantial sub-rounds in the 2026-05-13 evening session) is **also retired**. Network tier (Π1.1-Π1.5) + runtime-model (Π2.6-Π2.7) + node:* load-bearing breadth (Π3.8-Π3.13) + Bun-namespace load-bearing subset + reactor + external-fanout sources (file watchers, child pipes, signals, async DNS) all closed. Per Doc 715 §VII shift 3 + §X consolidation amendment, the apparatus has crossed L2M-saturation into the consolidation regime: empirical retirement-fanout 50-200× per substrate widening, new-substrate-edges-per-probe ≈ 0.3, zero new alphabet elements across the entire 2026-05-13 sweep. The five-surface ceiling analysis is structurally closed modulo adversarial-async-graph (successor-engagement scope per Doc 715 §X.c). The roadmap below is preserved as historical record. The live forward roadmap is held in trajectory.md — **two terms** (real-OSS basket / eval-ESM), dominated by basket expansion rather than substrate introduction.
